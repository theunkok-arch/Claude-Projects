...
- **`netlify/functions/portal.mjs` filtert server-side.** Voeg daar nooit een veld
  toe zonder te controleren of de klant het mag zien (README, "Klantportaal" en
  "Toegang en privacy"). Het oude `rapport.mjs` is vervallen; bouw geen tweede
  klantgericht oppervlak met een eigen veldfilter, want die lopen uit de pas. Nergens een `...fields`: elk veld dat
